...
A set itself is **mutable** (you can add/remove elements),
but every **element inside** must be **immutable**.
...
# Real usage from Cownomics:
if spotty_letters.isdisjoint(plain_letters):
    count += 1    # No common letter found
```
...
> ℹ️ **Note:** `help()` may not work correctly in some VS Code REPL modes.
...
d.values()  # dict_values(["Haider", 42, "Cairo"])
d.items()   # dict_items([("name", "Haider"), ...])
...
> ℹ️ **Note:** `dict_keys` and `dict_values` are memory-efficient views.
> Convert to list only when you need sorting or indexing.
...
Converts a character to its ASCII / Unicode number.
...
> ℹ️ **Note:** Very fast and common in competitive programming.
> Prefer a dictionary when readability matters more than performance.
...
Building strings with `+=` inside a loop is slow because strings are immutable —
each `+=` creates a completely new string.
...
Every `replace()` creates a completely new string.
Process each character once and build with `join()` instead.
...
A generator produces values **one at a time** without building a full list in memory.

```python
sum(1 for word in words if len(word) == 4)
```

- Uses less memory than a list comprehension.
...
| Need                                    | Best Structure |
|-----------------------------------------|----------------|
| Fast membership check / unique values   | `set`          |
